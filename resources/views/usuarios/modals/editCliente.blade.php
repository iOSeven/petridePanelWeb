    ...
    <div class="modal-dialog">
        <div class="modal-content" style="border-radius: 20px">
            ...
            <div class="modal-body">
                ...
                <form id="myForm" name="myForm" method="POST" action="{{ route('usuarios.update', $cliente->id) }}">
                    @csrf
                        ...
                        <div class="col-md-6">
                            <select class="form-control" id="tipo" name="tipo" required>
                                ...
                                @foreach($servicios as $pos => $servicio)
                                    <option value="{{ $servicio->id }}" {{ ( $servicio->id == $cliente->id_tipo_servicio) ? 'selected' : '' }}>{{ $servicio->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-6">
                            <select class="form-control" id="estatus" name="estatus" required>
                                <option value="">Seleccione tipo de estatus</option>
                                    <option value="Aprobado" {{ ($cliente->estatus == "Aprobado") ? 'selected' : '' }}>Aprobado</option>
                                    <option value="Pendiente" {{ ($cliente->estatus == "Pendiente") ? 'selected' : '' }}>Pendiente</option>
                                    <option value="Rechazado" {{ ($cliente->estatus == "Rechazado") ? 'selected' : '' }}>Rechazado</option>
                            </select>
                        </div>
                    </div>
